...
const { Topic, SubTopic, Track, Article } = require("../../models/common/topic.js");
const Selection = require("../../models/common/Selection.js");
const {
  validateTopicCreate,
  validateTopicUpdate,
  validateTopicList,
...
router.put("/", identityManager(["superAdmin"]), async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(topicId))
    return failure(res, req.apiId, TOPIC_CONSTANTS.INVALID_TOPIC_ID);
...
router.get(
  ...
  async (req, res) => {
    ...
    if (req.query.topicId)
      criteria._id = new mongoose.Types.ObjectId(req.query.topicId);
...
router.get(
  ...
  async (req, res) => {
    ...
    if (req.query.subTopicId)
      criteria._id = new mongoose.Types.ObjectId(req.query.subTopicId);
    if (req.query.type) criteria.type = req.query.type;
    if (req.query.text) {
      const regexText = new RegExp(req.query.text, "i");
      criteria.$or = [{ title: regexText }];
...
router.get(
  ...
  async (req, res) => {
    ...
    if (req.query.trackId)
      criteria._id = new mongoose.Types.ObjectId(req.query.trackId);
    if (req.query.type) criteria.type = req.query.type;
    if (req.query.text) {
      const regexText = new RegExp(req.query.text, "i");
      criteria.$or = [{ title: regexText }];
...
router.post("/article", identityManager(["superAdmin"]), async (req, res) => {
  ...
  const response = _.pick(newArticle.toObject(), ["_id", "name", "insertDate"]);
...
router.get(
  ...
  async (req, res) => {
    ...
    if (req.query.articleId)
      criteria._id = new mongoose.Types.ObjectId(req.query.articleId);
    if (req.query.type) criteria.type = req.query.type;
    if (req.query.text) {
      const regexText = new RegExp(req.query.text, "i");
      criteria.$or = [{ title: regexText }];
...
// // Save selected topics for a journal
// router.post(
//   "/save-selections",
//   identityManager(["user", "superAdmin", "admin"]),
//   async (req, res) => {
//     const { journalId, topicIds } = req.body;

//     try {
//       // 1. Delete old data (Unselect handling)
//       await Selection.deleteMany({ journalId: journalId });

//       // 2. Check if array is empty (User unselected all)
//       if (!topicIds || topicIds.length === 0) {
//         return res.json({ success: true, message: "All selections removed" });
//       }

//       // 3. Format data for bulk insert
//       const selectionData = topicIds.map((id) => ({
//         journalId: journalId,
//         topicId: id,
//       }));
...
// // Get selected topics for a journal
// router.get("/list-with-selection", async (req, res) => {
//   const { journalId } = req.query;

//   const list = await Topic.aggregate([
//     {
//       // Join Selection model with Topics
//       $lookup: {
//         from: "selections", // selection collection ka naam
//         let: { topicId: "$_id" },
//         pipeline: [
//           {
//             $match: {
...
//         minSelections: 1,
//         maxSelections: 1,
//         // Agar array ki length > 0 hai, matlab user ne select kiya hai
//         selected: { $gt: [{ $size: "$isSelected" }, 0] },
//       },
//     },
//     { $sort: { sortOrder: 1 } },
//   ]);
