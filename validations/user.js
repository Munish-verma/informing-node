...
function validateUserRegister(req) {
  const schema = Joi.object({
    ...
    role: Joi.string().valid("user", "eic", "admin", "reviewer", "publisher").required(),
    label: Joi.string().valid("senior", "associate", "guest", "managing", "").optional().allow(""),
  });
...
function validateUserEdit(req) {
  const schema = Joi.object({
    ...
    role: Joi.array().items(
      Joi.string().valid("user", "eic", "admin", "reviewer", "publisher")
    ).optional(),
    label: Joi.string().valid("senior", "associate", "guest", "managing", "").optional().allow(""),
